...
namespace FranchAdm
{
    public partial class OrderStatusPage : UserControl
    {
        ...
        private void LoadData()
        {
            try
            {
                ...
                using (var db = new FranchiseDBEntities1())
                {
                    ...
                    itemsTable.Columns.Add("OrderStatusId", typeof(int));
        ...
        {
            var editWindow = new OrderStatusEditWindow();
        ...
        private void BtnEdit_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (row["OrderStatusId"] == DBNull.Value) return;

                int id = Convert.ToInt32(row["OrderStatusId"]);
                var editWindow = new OrderStatusEditWindow(id);
                if (editWindow.ShowDialog() == true)
                {
                    LoadData();
                }
        ...
        private void BtnDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (lvItems.SelectedItem is DataRowView row)
            {
                ...
                try
                {
                    using (var db = new FranchiseDBEntities1())
                    {
                        // ✅ ПРОВЕРКА: используется ли статус в заявках
                        var ordersCount = db.Orders.Count(o => o.OrderStatusId == id);
                    ...
                    if (result == MessageBoxResult.Yes)
                    {
                        using (var db = new FranchiseDBEntities1())
                        {
                            ...
                            if (item != null)
                            {
                                ...
                                MessageBox.Show("Статус заказа успешно удалён!", "Успех",
