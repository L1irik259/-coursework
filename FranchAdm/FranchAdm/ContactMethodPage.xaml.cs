...
namespace FranchAdm
{
    public partial class ContactMethodPage : UserControl
    {
        ...
        private void LoadData()
        {
            try
            {
                ...
                using (var db = new FranchiseDBEntities1())
                {
                    ...
                    itemsTable.Columns.Add("ContactMethodId", typeof(int));
        ...
        {
            var editWindow = new ContactMethodEditWindow();
        ...
        private void BtnEdit_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (row["ContactMethodId"] == DBNull.Value) return;

                int id = Convert.ToInt32(row["ContactMethodId"]);
                var editWindow = new ContactMethodEditWindow(id);
                if (editWindow.ShowDialog() == true)
                {
                    LoadData();
                }
        ...
        private void BtnDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (lvItems.SelectedItem is DataRowView row)
            {
                ...
                try
                {
                    using (var db = new FranchiseDBEntities1())
                    {
                        // ✅ ПРОВЕРКА: используется ли способ связи в заявках
                        var ordersCount = db.Orders.Count(o => o.ContactMethodId == id);

                        if (ordersCount > 0)
                        {
                            MessageBox.Show(
                                $"Нельзя удалить способ связи \"{name}\"!\n\n" +
                    ...
                    if (result == MessageBoxResult.Yes)
                    {
                        using (var db = new FranchiseDBEntities1())
                        {
                            var item = db.ContactMethods.Find(id);
                            if (item != null)
                            {
                                db.ContactMethods.Remove(item);
                                db.SaveChanges();

                                MessageBox.Show("Способ связи успешно удалён!", "Успех",
                                    MessageBoxButton.OK, MessageBoxImage.Information);
                                LoadData();
                            }
                            else
                            {
                                MessageBox.Show("Способ связи не найден в базе данных.", "Ошибка",
